...
C-to-Rust migration SuperSpec stage orchestration — defines the 11-stage pipeline that the V2 orchestrator uses to dispatch subagents, validate input/output contracts, enforce permission boundaries, and control gating between phases. This is the second layer of V2's three-layer execution governance (OpenSpec + SuperSpec + SuperPower).
