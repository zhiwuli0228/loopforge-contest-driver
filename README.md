...
LoopForge Contest Driver 是一个面向无人值守执行和自动化评测的通用 **C/C++ 到 Rust 迁移 Harness**。它以预置的 `work/design/README.md` 为需求契约，以只读 `SOURCE_ROOT` 为源码输入，生成可构建、可测试的 Rust 工程，并通过编译自愈、测试迁移、语义审计和质量门禁验证迁移结果。

`work/code/` 仅用于本地测试，不属于正式提交包，也不是正式运行依赖。
...
- **输入项目自动发现**：支持传入 C/C++ 项目本身或其上层容器；根据翻译单元、构建文件、测试和目录结构定位唯一项目根目录。
- **预置设计驱动**：只从提交包内的 `work/design/README.md` 获取项目约束和验收上下文，并记录其 SHA-256。
...
运行时只要求注入 `SOURCE_ROOT`；任务信息已经在提交前固化到 `work/design/README.md`。
...
1. 校验 `work/design/README.md`，再根据源码和构建证据定位唯一 C/C++ 项目。
